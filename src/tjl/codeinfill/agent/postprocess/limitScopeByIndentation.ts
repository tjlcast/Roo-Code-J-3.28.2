...
import { getIndentationLevel, isBlank, isBlockClosingLine, isBlockOpeningLine, splitLines } from "../utils"
	...
	return (input: string, context: CompletionContext) => {
		...
		while (index < inputLines.length) {
			...
			if (indentLevel < indentContext.indentLevelLimit) {
				...
				if (
					isBlockClosingLine(inputLinesForDetection, index) &&
					isBlockOpeningLine(inputLinesForDetection, index)
				) {
					index++
					continue
				}
		...
		if (index < inputLines.length) {
			logger().debug("Remove content out of indent scope", {
				inputLines,
				prefixLines,
				suffixLines,
				scopeLineCount: index,
			})
