...
export function errorToString(error: Error & { cause?: Error }) {
	...
	if (error && error.cause && error.cause instanceof Error) {
		message += "\nCaused by: " + errorToString(error.cause as Error & { cause?: Error })
	}
	return message
}
...
export function findUnpairedAutoClosingChars(input: string): string[] {
	...
	while (index < input.length) {
		...
		let nextFound: {
			...
			found:
				| {
						pair: AutoClosingPair
						pos: AutoClosingCharPosition
						pattern: AutoClosingCharPattern
				  }
				| undefined
		} = {
			index: remain.length,
